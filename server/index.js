...
const fs = require("fs");
const conferenceStylesRouter = require('./routes/conferenceStyles');
...
app.use(conferenceStylesRouter);
...
// Сохранение стилей конференции
app.post('/api/conferences/:id/styles', async (req, res) => {
  try {
    const conferenceId = parseInt(req.params.id);
    const styles = req.body;
    
    // Убираем conference_id из тела запроса (если он там есть)
    delete styles.conference_id;
    
    // Добавляем updated_at
    styles.updated_at = new Date();
    ...
    const allowedFields = [
      ...
      'section_title_font_size', 'section_title_font_weight', 'section_title_color', 
    ...
    // Фильтруем только те поля, которые есть в таблице и пришли в запросе
    const updateFields = [];
    const values = [];
    let paramIndex = 1;
    ...
    // Добавляем conference_id в конец значений
    values.push(conferenceId);
    
    // SQL запрос с UPSERT (INSERT или UPDATE)
    const query = `
      INSERT INTO conference_styles (conference_id, ${updateFields.map(f => f.split(' = ')[0]).join(', ')})
      VALUES ($1, ${values.slice(0, -1).map((_, i) => `$${i + 2}`).join(', ')})
      ON CONFLICT (conference_id) 
      DO UPDATE SET ${updateFields.map(f => f.replace(/\$\d+/, match => {
        // Пересчитываем индексы для UPDATE части
        const index = parseInt(match.substring(1));
        return `$${index + updateFields.length + 1}`;
      })).join(', ')}
      RETURNING id
    `;
    
    // Упрощённая версия: сначала проверяем, есть ли запись
    const checkQuery = 'SELECT id FROM conference_styles WHERE conference_id = $1';
    const checkResult = await db.query(checkQuery, [conferenceId]);
    ...
    if (checkResult.rows.length === 0) {
      ...
      const insertQuery = `
        ...
        RETURNING id
      `;
      result = await db.query(insertQuery, [conferenceId, ...values.slice(0, -1)]);
    } else {
      // UPDATE
      const updateQuery = `
...
app.get('/api/conferences/:id/styles', async (req, res) => {
  try {
    const conferenceId = parseInt(req.params.id);
    ...
    if (result.rows.length === 0) {
      // Возвращаем пустой объект, если стили не найдены
      return res.json({ 
        success: true, 
        styles: {},
        message: 'Стили не найдены, будут использованы значения по умолчанию'
      });
    }
...
// ============================================
// ========== НОВЫЕ ЭНДПОИНТЫ ДЛЯ ДОКЛАДОВ ==========
// ============================================

// Сохранение доклада (с файлом)
app.post('/api/reports', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const { 
            title, abstract, keywords, additional_info, 
            content, literature, user_id, conference_id, authors 
        } = req.body;
        ...
        const reportResult = await client.query(
            `INSERT INTO reports (
                title, abstract, keywords, additional_info, 
                content, literature, file_name, file_path, file_size,
                user_id, conference_id, submitted_at
        ...
        for (let i = 1; i < authorsList.length; i++) {
            await client.query(
                `INSERT INTO coauthors (id_report, name, coauthor_order)
                 VALUES ($1, $2, $3)`,
                [reportId, authorsList[i], i]
            );
        }
        
        await client.query('COMMIT');
        console.log(`✅ Доклад сохранен с ID: ${reportId}`);
        
        res.json({ 
            success: true,
            id: reportId, 
            message: 'Доклад успешно сохранен' 
        });
...
app.post('/api/drafts', upload.single('file'), async (req, res) => {
    try {
        const {
            userId, title, abstract, keywords, content, literature,
            coauthors_data, additional_info, conference_id
        } = req.body;
        
        const file = req.file;
        
        const existingDraft = await pool.query(
            'SELECT id FROM report_drafts WHERE user_id = $1',
            [userId]
        );
        ...
        if (existingDraft.rows.length > 0) {
            result = await pool.query(
                `UPDATE report_drafts 
                 SET title = $1, abstract = $2, keywords = $3, 
                     content = $4, literature = $5, coauthors_data = $6,
                     additional_info = $7, conference_id = $8,
                     file_name = $9, file_path = $10,
                     last_modified = NOW()
                 WHERE user_id = $11
                 RETURNING id`,
                [
                    title, abstract, keywords, content, literature,
                    coauthors_data, additional_info, conference_id,
                    file ? file.originalname : null,
                    file ? file.path : null,
                    userId
            ...
            result = await pool.query(
                `INSERT INTO report_drafts (
                    user_id, title, abstract, keywords, content, literature,
                    coauthors_data, additional_info, conference_id,
        ...
        console.log(`✅ Черновик сохранен для пользователя ${userId}`);
...
app.get('/api/drafts/user/:userId', async (req, res) => {
    try {
        ...
        console.log(`✅ Загружен черновик для пользователя ${req.params.userId}`);
...
app.get('/api/reports/user/:userId', async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT r.*, 
                    u.name as main_author,
                    COALESCE(
                        (SELECT JSON_AGG(
                            JSON_BUILD_OBJECT(
                                'name', c.name,
                                'order', c.coauthor_order
                            ) ORDER BY c.coauthor_order
                        ) FROM coauthors c WHERE c.id_report = r.id),
                        '[]'::json
                    ) as coauthors
             FROM reports r
        ...
        res.status(500).json({ 
            success: false,
            error: 'Ошибка загрузки докладов',
            details: error.message 
        });
    }
});

// Получение конкретного доклада
app.get('/api/reports/:id', async (req, res) => {
    try {
            ...
            `SELECT r.*, 
                    ...
                    COALESCE(
                        (SELECT JSON_AGG(
                            JSON_BUILD_OBJECT(
                                'name', c.name,
                                'academic_degree', c.academic_degree,
                                'academic_title', c.academic_title,
                                'position', c.position,
                                'workplace', c.workplace,
                                'email', c.email,
                                'is_corresponding', c.is_corresponding,
                                'order', c.coauthor_order
                            ) ORDER BY c.coauthor_order
                        ) FROM coauthors c WHERE c.id_report = r.id),
                        '[]'::json
        ...
        console.log(`✅ Загружен доклад ID: ${req.params.id}`);
        ...
        console.error('❌ Ошибка загрузки доклада:', error);
        res.status(500).json({ 
            success: false,
            error: 'Ошибка загрузки доклада',
            details: error.message 
        });
    }
});

// Скачивание файла доклада
app.get('/api/reports/:id/download', async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT file_path, file_name FROM reports WHERE id = $1',
            [req.params.id]
        );
...
app.delete('/api/reports/:id', async (req, res) => {
    ...
    try {
        ...
        if (fileResult.rows[0]?.file_path) {
            fs.unlink(fileResult.rows[0].file_path, (err) => {
        ...
        console.log(`✅ Доклад ${req.params.id} удален`);
        
        res.json({ 
            success: true,
            message: 'Доклад успешно удален' 
        });
        
    } catch (error) {
        await client.query('ROLLBACK');
        console.error('❌ Ошибка удаления доклада:', error);
        res.status(500).json({ 
            success: false,
            error: 'Ошибка удаления доклада',
            details: error.message 
        });
    } finally {
        client.release();
